// Settings sheet (GDD §8.4): sound, hero rename + avatar, save
// export/import (emoji-safe base64), and a two-tap progress reset.
...
import { loadSave, mutateSave, persistSave } from '@discoveryquest/engine/save';
import { setSoundEnabled } from '@discoveryquest/voice-kit/audio';
import { setMusicEnabled, isMusicOn } from './music.js';
import { applyDocumentTitle } from './brand.js';
...
export default function SettingsSheet({ onClose, onGrownUps }) {
  ...
  function saveProfile() {
    mutateSave((s) => {
      if (name.trim()) s.profile.name = name.trim().slice(0, 18);
      s.profile.avatar = avatar;
    });
    applyDocumentTitle();
    onClose(true); // remount the map so brand/avatar refresh
  }
  ...
  async function copyExport() {
    try {
      ...
      /* clipboard unavailable — the string is still selectable below */
    }
  }
  ...
  function doReset() {
    ...
    localStorage.removeItem('lmq-save');
    window.location.reload();
  }
      ...
      <div className="pointer-events-none fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <button
            type="button"
            onClick={toggleSound}
            className="flex w-full items-center gap-3 rounded-2xl border border-white/10 bg-white/4 px-4 py-3 font-bold text-slate-200 transition-colors hover:bg-white/8"
          >
          ...
          <button
            ...
            onClick={() => {
              ...
              mutateSave((s2) => {
                s2.settings.music = next;
              });
            }}
            className="mt-2 flex w-full items-center gap-3 rounded-2xl border border-white/10 bg-white/4 px-4 py-3 font-bold text-slate-200 transition-colors hover:bg-white/8"
          >
            🎵 Music
            ...
            {AVATARS.map((a) => (
              <button
                ...
                className={`flex h-10 w-10 items-center justify-center rounded-xl border-2 text-xl transition-all ${
                  avatar === a ? 'border-cyan-300 bg-cyan-400/15' : 'border-white/10 bg-white/4'
                }`}
              >
                {a}
              </button>
          ...
          <div className="mt-2 flex gap-2">
            ...
            />
            <button type="button" onClick={saveProfile} className="rounded-xl bg-cyan-400 px-4 font-extrabold text-slate-900 hover:bg-cyan-300">
              Save
            </button>
          </div>
          ...
          <button
            type="button"
            onClick={copyExport}
            className="mt-2 flex w-full items-center gap-2 rounded-xl border border-white/10 bg-white/4 px-4 py-2.5 text-sm font-bold text-slate-300 hover:bg-white/8"
          >
          ...
          <div className="mt-2 flex gap-2">
            ...
            />
            <button type="button" onClick={doImport} disabled={!importStr.trim()} className="rounded-xl border border-white/15 bg-white/5 px-3 text-sm font-extrabold text-slate-200 hover:bg-white/10 disabled:opacity-40">
              Load
            </button>
          </div>
          ...
          {onGrownUps && (
            <button
              type="button"
              onClick={onGrownUps}
              className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/4 px-4 py-2.5 text-sm font-extrabold text-slate-300 hover:bg-white/8"
            >
          ...
          <button
            type="button"
            onClick={doReset}
            className={`mt-6 flex w-full items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-extrabold transition-colors ${
              confirmReset ? 'border-rose-400 bg-rose-500/20 text-rose-200' : 'border-rose-400/30 bg-rose-500/5 text-rose-300/80 hover:bg-rose-500/10'
            }`}
          >
            <RotateCcw size={15} />
