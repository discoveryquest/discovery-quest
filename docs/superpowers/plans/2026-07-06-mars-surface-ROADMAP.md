...
- **Last updated:** 2026-07-07, session_01UuNPTjgu6qHRcfjKrtqiQu (M5+M6 done; M7 T19+T21 done — only T20 mobile remains in M7)
- **Milestones done:** M0–M4, **M5 (wind + HUD gauges + gravity toggle)**, **M6 (real NASA Perseverance rover + fact card + lander + ElevenLabs wind/impact audio via own WebAudio graph)**, **M7 partial: T19 (branded loading + WebGL fallback), T21 (reduced-motion + watermarked snapshot)**. Plus telemetry HUD + scatter boulders. **Walking/jumping CONFIRMED WORKING by Pavel in-browser.** Tests 19/19 green; mars-preview build green.
- **Current task:** **T20 — mobile touch controls (the only unchecked M7 box).** `ui/Controls.jsx` (left virtual joystick → move, right-half drag → look, on-screen Jump + interact buttons) feeding the SAME `input/inputStore.js` path as keyboard; `ui/ControlsHint.jsx` (first-touch overlay that fades, portrait→landscape nudge). **Must be verified on a real phone** (Vite `host:true`, open the Network URL) — headless can't confirm touch feel. After T20: **M8** swap in Meshy-rigged Luna + rock glbs (get Pavel's OK on look first; `meshy_check_balance` first), **M9** ship (mirror check, ignore-check, deploy, curl assets, real-phone loop).
- **PENDING PAVEL DEVICE CHECKS:** (1) **listen to audio** — open localhost:5173, click once to arm sound, walk (wind swells with gusts), throw a rock at the ground/rover (thud); toggle 🔊. (2) throw a rock at the rover → should **bounce** (fixed collider). (3) mobile feel once T20 lands.
